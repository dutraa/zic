...
# create za_hashed_license secret file in /usr/bin/secrets/zic
#
# Gets the zapps user & password
#
# Example:
# create_za_hashed_license user password
#
create_za_hashed_license_secret(){
    ZIC_SECRETS_FOLDER=/usr/bin/secrets/zerto/zic
	USER_PASSWORD_CONCAT="$1:$2"
	sudo mkdir -p $ZIC_SECRETS_FOLDER
	sudo chmod -R 707 $ZIC_SECRETS_FOLDER
	echo -n $USER_PASSWORD_CONCAT | base64 -w 0 | openssl dgst -binary -sha256 | base64 -w 0 > $ZIC_SECRETS_FOLDER/za_hashed_license
	sudo chmod -R 700 $ZIC_SECRETS_FOLDER
	sudo chmod 600 $ZIC_SECRETS_FOLDER/za_hashed_license
}
